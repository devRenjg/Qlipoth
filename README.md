...
`版本 v20260707.1` · `Python 3.12 + FastAPI` · `Vue 3 + Vite`
...
- **权限隔离**：三级角色（管理员 / 超级用户 / 普通用户）+ 访客只读；未登录即可浏览，写/管理操作服务端鉴权
...
- 日历视图：按日聚合的直播热度看板（过去看实际峰值 / 未来看预约，双口径分离）；接入审批中台"直播报备"单提前标注保障预判（可插拔，公开版默认不含内部对接细节）；赛事直播间标题按真实赛程还原为当天对阵+比分；高热场次（百万级 PCU）补充观看时长/弹幕/进房DAU/涨粉双口径指标；场次视觉分级 + 长期持续流标注 + 每日自动维护（自愈补缺 + 数据完整性复核）
- 临时任务体系：与知识库隔离的一次性外部分析任务，独立 Tab 展示 + 文档导出

### 工程与质量
- 三级角色权限（管理员 / 超级用户 / 普通用户）+ 访客只读态，服务端鉴权 + 水平越权防护
- 用户行为日志埋点（功能使用统计）
...
- 只读内容对访客开放，写/管理接口服务端鉴权（访客与未登录写操作 401 / 越权 403），前端隐藏不作为安全边界
