...
{{"items": [
  {{"dimension": "高可用保障", "phenomenon": "现象(发生了什么)", "cause": "原因", "handling": "当时如何处置/解决", "suggestion": "给下一届的建议", "timing": "该提前到什么时点做(没有则空串)"}}
]}}
dimension 必须逐字取自六维度之一。"""
            ...
            for i, it in enumerate(all_items):
                await db.execute(
                    """INSERT INTO checklist_items
                       (checklist_id, dimension, phenomenon, cause, handling, suggestion, timing, source_files, sort_order)
                       VALUES (?,?,?,?,?,?,?,?,?)""",
                    (checklist_id, it["dimension"], it["phenomenon"], it["cause"],
                     it["handling"], it["suggestion"], it["timing"], it["source_file"], i))
            await db.execute(
...
async def get_checklist(checklist_id: int, request: Request):
    ...
    return {"checklist": dict(cl), "dimensions": DIMENSIONS,
...
async def update_item(item_id: int, req: ItemUpdate, request: Request):
    await _require_admin(request)
    fields, vals = [], []
    for k in ("dimension", "phenomenon", "cause", "handling", "suggestion", "timing"):
        v = getattr(req, k)
        if v is not None:
            fields.append(f"{k} = ?")
            vals.append(v)
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cur = await db.execute(
            """INSERT INTO checklist_items
               (checklist_id, dimension, phenomenon, cause, handling, suggestion, timing, source_files, sort_order)
               VALUES (?,?,?,?,?,?,?,?,?)""",
            (checklist_id, req.dimension, req.phenomenon, req.cause, req.handling,
             req.suggestion, req.timing, "(手动添加)", mx + 1))
        await db.commit()
