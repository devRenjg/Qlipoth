...
- team 负责团队/业务线：根据文档标题与内容判定该坑所属团队或业务线（如 流媒体、基架、弹幕、OTT/TV端、产品、研发、质量保障、直播体验、带宽/成本、安全 等），尽量填、判不出才留空
- owner 负责人：仅当内容里出现**真实中文姓名**(如 张三/李雷)时填写；遇到脱敏ID(如 @eJP3、@autoZF 这类字母数字乱码)、字体名(微软雅黑)、纯数字，一律**留空**，绝不把这些当负责人

返回严格 JSON（不要 markdown）：
{{"items": [
  {{"dimension": "高可用保障", "stage": "压测演练", "team": "流媒体", "owner": "", "phenomenon": "现象(发生了什么)", "cause": "原因", "handling": "当时如何处置/解决", "suggestion": "给下一届的建议", "timing": "该提前到什么时点做(没有则空串)"}}
]}}
