...
要求：
① 先用一句话客观说明这批表格数据是关于什么的（仅依据表头/字段名如实概括，不得添加表格里没有的信息、不得修饰演绎），作为第一行，以"表格数据关于："开头。
② 然后每条记录一行，不遗漏关键字段（名称/数值/负责人/状态/时间等）。
③ 忠实于原文，只做客观陈述，不解释、不评价、不补充背景。只输出上述内容，不要markdown。"""
