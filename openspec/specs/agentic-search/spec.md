...
### Requirement: 客观事实命中评测

检索质量评测 SHALL 提供基于"标准答案事实点(golden)"的客观度量：出题时同时生成关键事实点(来自原文、可核对)，评委仅核对命中数量与正确性、惩罚事实错误，不因篇幅/文采/完整度加分。评测 SHALL 盲评(随机化A/B、不暴露新旧)，按维度拆解输出命中率，检索质量与效率分开呈现。

#### Scenario: 啰嗦但无新增事实不加分

- **WHEN** 两答案命中相同事实点，其一更长含更多无关背景
- **THEN** 事实命中得分相同，不因更长判优

### Requirement: 知识库文件读取路径边界

读取知识库文件的入口 SHALL 将路径限定在 KB 根目录内(resolve+is_relative_to)，拒绝绝对路径/`..`/空路径/非 .md；辅助入口按 basename+后缀校验。

#### Scenario: 路径穿越被拒
- **WHEN** 读取 `../config.py`、绝对路径或非 .md
- **THEN** 拒绝返回空/404，不越界读取

### Requirement: 不可信文档渲染净化

知识库内容(上传/导入/抓取)属不可信输入，渲染为 HTML 的页面 SHALL 在写入 DOM 前 DOMPurify 净化、插值转义，防 XSS。

#### Scenario: 恶意脚本不执行
- **WHEN** 文档含 `<script>`/`onerror`/`javascript:`
- **THEN** 渲染后不执行、危险属性被净化

### Requirement: 评测集随内容演进重审 + 新旧库语义对比结论

Golden Set SHALL 随知识库演进重审、覆盖反映新能力的维度(表格/跨文档/精确数值)。受控对比结论：API 导入相对 Playwright 旧版，grep 词法检索下提升微弱(Recall+0.02，旧库冗余噪声反易命中)，BM25 语义检索下全面领先(Recall+0.18/MRR+0.20，全维度胜)——结构化导入价值需语义检索体现，是检索层演进方向。

#### Scenario: 语义检索体现结构化导入价值
- **WHEN** 同受控对比下分别用词法/语义检索评测新旧库
- **THEN** 语义检索下新库提升显著大于词法检索

### Requirement: 图片代理接口鉴权与 SSRF 防护

服务端代理图片的接口(/documents/img-proxy、/documents/kb-image)SHALL 要求登录。带服务端 cookie 回源的 img-proxy SHALL 强制 https + host 严格白名单 + 禁止跟随重定向(防 cookie 被带至非白名单 host)。

#### Scenario: 匿名访问图片代理被拒
- **WHEN** 未登录请求 img-proxy 或 kb-image
- **THEN** 返回 401

#### Scenario: 非白名单/非https目标被拒
- **WHEN** img-proxy 收到非 https 或非白名单 host 的 url，或回源遇重定向
- **THEN** 拒绝(400/502)，不向非白名单 host 发带 cookie 请求
